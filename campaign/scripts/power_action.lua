...
function onInit()
	...
	DB.addHandler(DB.getPath(DB.getChild(node, "....."), "spellcheck_total"), "onUpdate", onDataChanged);
	onDataChanged();
end
...
function onClose()
	...
	DB.removeHandler(DB.getPath(DB.getChild(node, "....."), "spellcheck_total"), "onUpdate", onDataChanged);
end
...
function onCastChanged()
	local sCast, sSave = PowerManager.getPCPowerCastActionText(getDatabaseNode());
	castview.setValue(sCast);
