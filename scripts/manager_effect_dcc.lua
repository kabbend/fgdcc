...
function onEffectActorStartTurn(nodeActor, nodeEffect)
	...
	for _,sEffectComp in ipairs(aEffectComps) do
		local rEffectComp = parseEffectComp(sEffectComp); Debug.console(rEffectComp);
		-- Conditionals
		...
		elseif rEffectComp.type == "DMGO" or rEffectComp.type == "REGEN" then
			...
			if nActive == 2 then
				if rEffectComp.type == "REGEN" and (ActorManager2.getPercentWounded("ct", nodeActor) >= 1) then
					break;
				end
...
function applyOngoingDamageAdjustment(nodeActor, nodeEffect, rEffectComp)
	...
	if rEffectComp.type == "REGEN" then		
		local nPercentWounded = ActorManager2.getPercentWounded("ct", nodeActor);
...
function evalAbilityHelper(rActor, sEffectAbility)
	...
	elseif sTag == "AGI" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "agility");
	elseif sTag == "STA" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "stamina");
	elseif sTag == "INT" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "intelligence");
	elseif sTag == "PER" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "personality");
	elseif sTag == "LUCK" then
		nAbility = ActorManager2.getAbilityBonus(rActor, "luck");
