...
function resetPowers(nodeCaster, bLong)
	...
	-- restore all powers
        for _,nodePower in pairs(DB.getChildren(nodeCaster, "powers")) do
             DB.setValue(nodePower, "is_lost", "number", 0);
        end

        -- reset disapproval range
        DB.setValue( nodeCaster, "disapprovalrange", "number", 1 );
...
function getPCPowerAction(nodeAction, sSubRoll)
	...
	if rAction.type == "cast" then
		...
		rAction.powerlevel = DB.getValue(nodeAction.getChild("..."), "level", 0);
		
		rAction.dice = DB.getValue(nodeAction, "castdice", {"d20"});
		rAction.modifier = DB.getValue(nodeAction, "castmod", 0);
		
		local sCastBase = DB.getValue(nodeAction, "castbase", "");
		if sCastBase == "fixed" then
			rAction.base = "fixed";
		elseif sCastBase == "ability" then
			rAction.base = "";
			rAction.stat = DB.getValue(nodeAction, "caststat", "");
		else
			rAction.base = "group";
			rAction.stat = DB.getValue(nodeAction, "caststat", "");
			rAction.stat2 = DB.getValue(nodeAction.getChild("....."), "spellstat", "");
		end
		...
		if sSaveType ~= "" then
			...
			if sSaveBase == "fixed" then
				...
				rAction.savebase = "check";
				rAction.savemod = DB.getValue(nodeAction.getChild("....."), "spellcheckresult", 0);
			end
		else
			rAction.save = "";
		end
...
function getPCPowerCastActionText(nodeAction)
	...
	if rAction then
		...
		sCast = StringManager.convertDiceToString(rAction.dice);
		if rAction.modifier ~= 0 then
			sCast = string.format("%s%+d", sCast, rAction.modifier);
		end
		
		if (rAction.save or "") ~= "" then
			sSave = StringManager.capitalize(DataCommon.save_ltos[rAction.save]:lower());
			if rAction.savebase == "fixed" then
				sSave = sSave .. " DC " .. rAction.savemod;
			else
				sSave = sSave .. " vs. check";
			end
...
function getPowerGroupRecord(rActor, nodePower)
	...
	if sActorType == "pc" then
		...
		if nodePowerGroup then
			...
			aPowerGroup.sCasterType = DB.getValue(nodePowerGroup, "castertype", "");
		end
	end
...
function evalAction(rActor, nodePower, rAction)
	...
	if rAction.type == "cast" then
		if (rAction.base or "") == "group" then
			...
			if aPowerGroup then
				rAction.castertype = aPowerGroup.sCasterType;
			end
			rAction.modifier = (rAction.modifier or 0) + DB.getValue(nodePower.getChild("..."), "spellcheck_total", 0);
		end
	...
	if (rAction.type == "effect") then
		...
		rAction.sName = EffectManagerDCC.evalEffect(rActor, rAction.sName);
	end
end
...
function performAction(draginfo, rActor, rAction, nodePower)
	...
	if rAction.type == "cast" then
		...
		-- end
		elseif ((rAction.subtype == "") or (rAction.subtype == "save")) and ((rAction.save or "") ~= "") then
